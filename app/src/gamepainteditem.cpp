...
void GamePaintedItem::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if (m_nLocalPlayers[m_pGame->turn()] == nullptr)
        return;
    if (0 > event->position().rx() / getCellWidth() || 0 > event->position().ry() / getCellHeight())
        return;
    if (m_mousePressPoint == QSize(event->position().rx() / getCellWidth(), event->position().ry() / getCellHeight()))
